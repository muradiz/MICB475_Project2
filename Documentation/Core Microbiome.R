...
core_6m <- four_way_6m + scale_x_continuous(expand = expansion(mult = .2)) + labs(caption = "6-month-old infants", fill = "# Genera") + theme(plot.caption = element_text(size = 12, hjust=0.5), plot.margin = margin(r = 20)) + scale_fill_gradient(low = "#EBE8FC", high = "#756BB1") + border() 
core_6m
#ggsave("four_way_6m(0.80prev).png", plot = last_plot(), height = 3.5, width = 5, dpi = 300)
...
core_12m
#ggsave("four_way_12m(0.80prev).png", plot = last_plot(), height = 3.5, width = 5, dpi = 300)

ggarrange(core_6m, core_12m, ncol = 2, nrow = 1) + bgcolor("white")

#ggsave("combinedcore_plot(0.80prev).png", plot = last_plot(), height = 7, width = 15, dpi = 300)
